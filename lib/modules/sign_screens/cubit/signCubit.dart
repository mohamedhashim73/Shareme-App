...
class SignCubit extends Cubit<SignStates>{
  ...
  void createUser({required String email,required String password,required String userName}){
    ...
    FirebaseAuth.instance.createUserWithEmailAndPassword(email: email, password: password).then((val){
      ...
      saveUserData(userName: userName, email: email, uid: val.user!.uid);      // save UserData on Cloud FireStore
    }).catchError((e){
      print(e.toString());
